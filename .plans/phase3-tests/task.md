...
**보장할 동작**: push / PR on `main` 에서 type-check + lint + test 자동 실행.
...
- `npm run lint` 가 현재 `--max-warnings=0` 설정 + 기존 코드 warning 70 개로 실패. T8 CI 에서 `lint` step 이 빨간불. 본 PR 에서 lint 이슈를 전부 해결하는 건 스코프 누수 → T8 시점에 ① lint step 을 CI 에서 잠시 `continue-on-error` 로 두고 별도 이슈로 분리, 또는 ② warning 0 만들기 별도 PR 중 선택.
